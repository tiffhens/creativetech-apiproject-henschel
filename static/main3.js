// This is the javascript for page 3 of the site

$.getJSON("https://api.tumblr.com/v2/tagged?tag=puppy&limit=20&filter=text&api_key=fuiKNFp9vQFvjLNvx4sUwti4Yb5yGutBN4Xh10LXZhhRKjWlV4&callback=?", function(theDataReceived) { //theDataReceived is the placeholder for the data
	...
	for(var i=0; i<theDataReceived.response.length; i++) {
		...
		var puppyPhoto = theDataReceived.response[i].photos;

		if(puppyPhoto !== undefined) {
			puppyPhotos.push(puppyPhoto);
		}
	}

	if(puppyPhotos.length > 0) {
		var random = Math.floor(Math.random() * puppyPhotos.length);
		var randomPost = puppyPhotos[random];
		$('#puppyimage').append("<img src='" + randomPost[0].original_size.url + "' />");
	}
